...
function interpolate_at_1(node2,node3,node5,node4,node1)
    ...
    if (Δy>1e-5 || Δx>1e-5)
        print("Interpolating the C+ point");
        #iterate till convergence
        for i in 1:10
            λplus=tan(node1[:θ]+node1[:α]);
            if (Δx<1e-5)
                node1[:x]=node3[:x];
                node1[:y]=node4[:y]-λplus*(node4[:x]-node1[:x]);
            else
                λ53=(node5[:y]-node3[:y])/(node5[:x]-node3[:x]);
                coeff_mat=[1 -λ53;1 -λplus];
                const_mat=[node3[:y]-λ53*node3[:x];node4[:y]-λplus*node4[:x]];
...
function compute_new_point_interior(node2,node3,node5,node4,node1,nodeplus,nodeminus,node0,c)
    ...
    λ0=tan(node0[:θ]);
    λminus=tan(nodeminus[:θ]-nodeminus[:α]);
    coeff_matrix=[1 -λ0;1 -λminus];
    const_matrix=[node3[:y]-λ0*node3[:x];node2[:y]-λminus*node2[:x]];
    p=coeff_matrix\const_matrix;
    node4[:y]=p[1];node4[:x]=p[2];
    interpolate_at_1(node2,node3,node5,node4,node1);
    #calculate p4, θ4
    Q_minus=Q(nodeminus);T_minus=T(node4,node2,nodeminus,Q_minus,-1);
    if c==1
        Q_plus=Q(nodeplus);
        T_plus=T(node4,node1,nodeplus,Q_plus,1);
    else
        Q_plus=Q(node1);
        T_plus=T(node4,node1,node1,Q_plus,1);
    end
...
function compute_new_Cminus(node2,node3,node5)
    ...
    for i=1:n
        ...
        V[i]=node4[:V];
        α[i]=node4[:α];
        node_plus=update(node_plus,node1,node4);
        node_minus=update(node_minus,node2,node4);
        node0=update(node0,node3,node4);
    end
    if (abs(x[end]-x[end-1])<1e-5 &&
        abs(y[end]-y[end-1])<1e-5 &&
        abs(θ[end]-θ[end-1])<1e-5 &&
        abs(V[end]-V[end-1])<1e-2 &&
        abs(α[end]-α[end-1])<1e-5 &&
        abs(pressure[end]-pressure[end-1])<1e-5 &&
        abs(density[end]-density[end-1])<1e-5 &&
        abs(temp[end]-temp[end-1])<1e-5)
        print("\nConvergence Successful-Interior point\n")
    end
    #Convergence monitor
    p1=plot(iterations,pressure,title="Interior point along Cminus", xlabel="Iterations", ylabel="Pressure",linecolor="green",lw=3);
...
# gas2.TPX=2700,34.730e5,[0.0,0.22,0.0,0.0,0.00,0.0,0.78,0.0];
# node2=Dict(:x=>0,:y=>0.75,:V=>1.001*af(gas2),:θ=>0.01*pi/180,:gas=>gas2);
# gas3=ct.Solution("air.xml");
# # gas3.TPX=1500,56446.3,[0,0.49342,0,0.00402,0.00633,0.00007,0.49615,0.00001];
# gas3.TPX=2700,34.730e5,[0.0,0.22,0.0,0.0,0.00,0.0,0.78,0.0];
# node3=Dict(:x=>0,:y=>0.5,:V=>1.001*af(gas3),:θ=>0.01*pi/180,:gas=>gas3);
# gas5=ct.Solution("air.xml");
# # gas5.TPX=999.9997532924206,56446.24320660717,[0.0, 0.4934199999999999, 0.0, 0.004019999999999999, 0.00633, 7.0e-5, 0.49615, 9.999999999999999e-6];
# gas5.TPX=2700,34.730e5,[0.0,0.22,0.0,0.0,0.00,0.0,0.78,0.0];
# node5=Dict(:x=>0.05,:y=>0.25,:V=>1.01*af(gas5),:θ=>0.0*pi/180,:gas=>gas5);
...
# node4=compute_new_Cminus(node2,node3,node5);
